...
<body>
  ...
  <div class="container">
    <table>
      ...
      <tbody>
        <?php
        $consulta = "SELECT id_ORDEN, Nombre FROM orden_servicio";
        ...
        while ($row = mysqli_fetch_assoc($result_consul)) { ?>
          <tr>
            ...
            <td id="orden_servicio">
              <?php echo $row['Nombre'] ?>
            </td>
            <td>
              <a href="../conexion/editarOrden.php?id=<?php echo $row['id_ORDEN'] ?>">
                <i class='bx bx-edit-alt' id="icon_editar"></i>
              </a>
            </td>
            <td>
              <a href="../conexion/eliminarOrden.php?id=<?php echo $row['id_ORDEN'] ?>" class="btn btn-danger">
